...
export async function translateVideo({
  ...
  try {
    ...
    if (selectedEndTime <= selectedStartTime) {
      throw new Error(
        "Set a valid translation start and finish time before processing.",
      );
    }
    ...
    const completeAnalysisResults = await analyseFrames({
      frames,
      sourceLanguage,
    });

    const analysisResults = completeAnalysisResults
      .filter(
        (result) =>
          result.timestamp < selectedEndTime &&
          result.timestamp + SAMPLE_INTERVAL_SECONDS > selectedStartTime,
      )
      .map((result) => ({
        ...result,
        timestamp: Math.max(
